...
enum State {
	...
	CAREFUL_IDLE, CAREFUL_WALK, CAREFUL_SWITCH
}
...
func process() -> void:
	match current_state:
		State.IDLE, State.WALK, State.SWITCH, State.CAREFUL_IDLE, State.CAREFUL_WALK, State.CAREFUL_SWITCH:
...
func _process_action_state() -> void:
	...
	if current_facing != previous_facing:
		change_state("SWITCH" if not is_careful else "CAREFUL_SWITCH")
	
	if current_state in [State.SWITCH, State.CAREFUL_SWITCH]:
		return
...
func handle_animation_finished() -> void:
	match current_state:
		...
		State.CAREFUL_SWITCH:
			change_state("CAREFUL_IDLE" if cha.input.get_input_direction() == Vector2.ZERO else "CAREFUL_WALK")
